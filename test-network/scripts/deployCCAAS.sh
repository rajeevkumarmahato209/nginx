...
startDockerContainer() {
  ...
  if [ "$CCAAS_DOCKER_RUN" = "true" ]; then
    ...
    set -x
    ${CONTAINER_CLI} run --rm -d --name peer0rajeev_${CC_NAME}_ccaas  \
                  --network fabric_test \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest

    ${CONTAINER_CLI} run  --rm -d --name peer0kumar_${CC_NAME}_ccaas \
                  --network fabric_test \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest
    ...
    infoln "    ${CONTAINER_CLI} run --rm -d --name peer0rajeev_${CC_NAME}_ccaas  \
                  --network fabric_test \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
    infoln "    ${CONTAINER_CLI} run --rm -d --name peer0kumar_${CC_NAME}_ccaas  \
                  --network fabric_test \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
...
## Install chaincode on peer0.rajeev and peer0.kumar
infoln "Installing chaincode on peer0.rajeev..."
installChaincode 1
infoln "Install chaincode on peer0.kumar..."
installChaincode 2

## query whether the chaincode is installed
queryInstalled 1

## approve the definition for rajeev
approveForMyOrg 1

## check whether the chaincode definition is ready to be committed
## expect rajeev to have approved and kumar not to
checkCommitReadiness 1 "\"rajeevMSP\": true" "\"kumarMSP\": false"
checkCommitReadiness 2 "\"rajeevMSP\": true" "\"kumarMSP\": false"

## now approve also for kumar
approveForMyOrg 2

## check whether the chaincode definition is ready to be committed
## expect them both to have approved
checkCommitReadiness 1 "\"rajeevMSP\": true" "\"kumarMSP\": true"
checkCommitReadiness 2 "\"rajeevMSP\": true" "\"kumarMSP\": true"
