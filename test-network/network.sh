...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating rajeev Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-rajeev.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating kumar Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-kumar.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/rajeev/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generating CCP files for rajeev and kumar"
...
# call the script to create the channel, join the peers of rajeev and kumar,
...
function networkDown() {
  ...
  # stop org3 containers also in addition to rajeev and kumar, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.example.com docker_peer0.rajeev.example.com docker_peer0.kumar.example.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/rajeev/msp organizations/fabric-ca/rajeev/tls-cert.pem organizations/fabric-ca/rajeev/ca-cert.pem organizations/fabric-ca/rajeev/IssuerPublicKey organizations/fabric-ca/rajeev/IssuerRevocationPublicKey organizations/fabric-ca/rajeev/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/kumar/msp organizations/fabric-ca/kumar/tls-cert.pem organizations/fabric-ca/kumar/ca-cert.pem organizations/fabric-ca/kumar/IssuerPublicKey organizations/fabric-ca/kumar/IssuerRevocationPublicKey organizations/fabric-ca/kumar/fabric-ca-server.db'
